"""运行 Raw/SFT validation benchmark，并生成可序列化的 Gate 报告。"""

from dataclasses import asdict, is_dataclass
from typing import Any, Iterable

from .validation_gate import BenchmarkSummary, SFTValidationGate
...
def compare_raw_sft_validation(
    *,
    ...
    """用同一 validation 协议比较 Raw/SFT，并返回 Gate 决策。"""
    validation_task_ids = list(task_ids)
    ...
    raw_summary = BenchmarkSummary(**raw_report["summary"])
    sft_summary = BenchmarkSummary(**sft_report["summary"])
    decision = (gate or SFTValidationGate()).decide(
        raw=raw_summary,
        sft=sft_summary,
    )
